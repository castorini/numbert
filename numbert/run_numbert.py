...
from .utils_trank import (compute_metrics, convert_examples_to_features,
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for ind, guid in enumerate(lguids):
            if out_label_ids[ind] == 1:
                if guid[1] in trank_labels:
                    trank_labels[guid[1]].add(guid[3])
                else:
                    trank_labels[guid[1]] = {guid[3]}
            if guid[1] in trank_predictions:
                trank_predictions[guid[1]].append((guid[3], preds[ind][1]))
            else:
                trank_predictions[guid[1]] = [(guid[3], preds[ind][1])]

        for key in trank_predictions:
            trank_predictions[key] = list(map(lambda x: x[0], sorted(trank_predictions[key], key=lambda tup: tup[1], reverse=True)))
            if key not in trank_labels:
                trank_labels[key] = set()
        eval_loss = eval_loss / nb_eval_steps
        # if args.output_mode == "classification":
        #     preds = np.argmax(preds, axis=1)
        # elif args.output_mode == "regression":
        #     preds = np.squeeze(preds)
        result = compute_metrics(eval_task, trank_predictions, trank_labels)
        ...
        if args.msmarco_output:
            ...
            with open(output_msmarco_file, "w") as f_msmarco:
                for query_id in trank_predictions:
                    rank = 1
                    for doc_id in trank_predictions[query_id]:
